...
1. **Разделение ответственности (UI vs Логика).** 
   Всё управление интерфейсом (тумблеры в карточках, загрузка фото, бейджики) остаётся в легковесном Node.js-плагине (`plugins/face-id/index.ts`) внутри ядра. Python-сервис занимается **исключительно** распознаванием и вообще не генерирует HTML/UI.
