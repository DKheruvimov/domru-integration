...
### 12. Внешние Модули и Плагины (External Modules Subsystem)

Ядро поддерживает подключение высокопроизводительных внешних плагинов (например, модулей компьютерного зрения) через API и WebSockets. Наглядным примером является разработанный **Face ID модуль** (`plugins/face-id/main.py`), написанный на Python.

**Архитектура взаимодействия:**
- **Регистрация возможностей (Dynamic Capabilities)**: При запуске внешний модуль выполняет рукопожатие с Ядром и регистрирует свои возможности (например, `FACE_RECOGNITION`). В интерфейсе Ядра для каждого человека в списке "Люди" динамически появляется тумблер включения этой возможности.
- **Изолированное постоянное хранилище (Isolated Storage)**: Модули могут сохранять и считывать бинарные данные (например, фотографии лиц жильцов) в изолированные папки под управлением Ядра (`/data/modules-storage/:moduleId/:key`), что обеспечивает персистентность.
- **Синхронизация данных (Real-time Sync)**: При подключении по WebSocket на пространство имён `/modules`, Ядро транслирует модулю событие `people_updated` при любых изменениях в базе жильцов. Модуль запрашивает активный список людей, у которых включена его возможность, и синхронизирует свою локальную базу.
- **Оптимизация трафика (Pre-check Image Hashing)**: Чтобы избежать избыточного скачивания фотографий жильцов, Ядро вычисляет хэш изображения (`imageHash`) на лету. Внешний модуль сравнивает присланный хэш со своим локальным кэшем (`processed_profiles.json`) и запрашивает файл заново только при реальном изменении.
- **Управление статусами (Entity-level status feedback)**: Внешний модуль отправляет Ядру детальные статусы обработки для каждого жильца (`processing`, `success`, `error`, `disabled`), которые отображаются в веб-интерфейсе в виде статусных индикаторов (бейджей) на карточке человека.
- **Конфликт сокет-соединений (Connection Conflict Resolution)**: Пространство имён `/modules` контролирует, чтобы под одним токеном был подключен только один экземпляр плагина. При обнаружении дубликата старый сокет принудительно отключается.
